...
# mocha_hash
RUN git clone https://github.com/mochapay/mocha_hash
RUN cd mocha_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} mocha
RUN useradd -u ${USER_ID} -g mocha -s /bin/bash -m -d /mocha mocha
...
RUN mkdir /mocha-src && \
  ...
  chown $USER_ID:$GROUP_ID /mocha-src && \
