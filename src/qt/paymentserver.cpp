...
//
// OSX-specific way of handling mocha: URIs and PaymentRequest mime types.
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom mocha addresses are not supported
        ...
        // Mocha amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
