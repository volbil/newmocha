...
const char * const BITCOIN_CONF_FILENAME = "mocha.conf";
const char * const BITCOIN_PID_FILENAME = "mochad.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MochaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MochaCore
    // Mac: ~/Library/Application Support/MochaCore
    // Unix: ~/.mochacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MochaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MochaCore";
#else
    // Unix
    return pathRet / ".mochacore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty mocha.conf if it does not excist
