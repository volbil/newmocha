...
#if defined(NDEBUG)
# error "Mocha Core cannot be compiled without assertions."
#endif
                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in Mocha
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
  //  if (!CheckProofOfWork(IsYesPower() ? block.GetPoWHash() : block.GetHash(), block.nBits, consensusParams))
...
static int64_t nTimeMochaSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeMochaSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Mocha specific: %.2fms [%.2fs]\n", 0.001 * (nTime5 - nTime4), nTimeMochaSpecific * 0.000001);
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
   ...
   // Get prev block index
    ...
    if (fCheckPOW && !CheckProofOfWork(IsYesPower(nHeight) ? block.GetPoWHash() : block.GetHash(), block.nBits, consensusParams))
