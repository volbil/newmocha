...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // users ignore all instructions on mochacentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in mocha.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("mocha.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in mocha.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Mocha features */
    { "mocha",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "mocha",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "mocha",               "gobject",                &gobject,                true,  {} },
    { "mocha",               "voteraw",                &voteraw,                true,  {} },
