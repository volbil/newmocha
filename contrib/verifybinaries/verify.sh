...
###   mochacore.org and mocha.org and compares them.
...
WORKINGDIR="/tmp/mocha_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://mochacore.org"
HOST2="https://mocha.org"
BASEDIR="/bin/"
VERSIONPREFIX="mocha-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'mocha-',
...
if [ $? -ne 0 ]; then
   echo "mocha.org failed to provide signature file, but mochacore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "mocha.org and mochacore.org signature files were not equal?"
