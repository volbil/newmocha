...
Mocha now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
Format: ```mngovernance prepare proposal-name url payment-count block-start mocha-address monthly-payment-mocha```
...
Format: ```mngovernance submit proposal-name url payment-count block-start mocha-address monthly-payment-mocha fee-tx```
...
The following RPC commands are supported:
 ...
 - mngovernance "command"... ( "passphrase" )
  ...
  - vote               - Vote on a proposal by single masternode (using mocha.conf setup)
