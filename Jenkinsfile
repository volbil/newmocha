...
// which are part of the Mocha repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="mocha-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /mocha-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "mochapay-mocha/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from mochapay-mocha/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'mochapay-mocha/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from mochapay-mocha/develop"
            }
          }

          if (hasCache) {
            sh "cd /mocha-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /mocha-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /mocha-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /mocha-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /mocha-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /mocha-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /mocha-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
